...
func newConfig(clusterHosts []string) *kafka.ConfigMap {
...
func (p *Producer) Produce(data string, topic string) error {
	...
	case <-time.After(5 * time.Second):
		return fmt.Errorf("event time out, make sure that Kafka is up and running")
	}
}
